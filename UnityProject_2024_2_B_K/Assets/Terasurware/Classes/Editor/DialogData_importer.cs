...
public class DialogData_importer : AssetPostprocessor {
	...
	private static readonly string[] sheetNames = { "DialogData", };
	...
	static void OnPostprocessAllAssets (string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
	{
		foreach (string asset in importedAssets) {
			...
			Entity_DialogData data = (Entity_DialogData)AssetDatabase.LoadAssetAtPath (exportPath, typeof(Entity_DialogData));
			if (data == null) {
				data = ScriptableObject.CreateInstance<Entity_DialogData> ();
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					Entity_DialogData.Sheet s = new Entity_DialogData.Sheet ();
					s.name = sheetName;
					...
					for (int i=1; i<= sheet.LastRowNum; i++) {
						...
						Entity_DialogData.Param p = new Entity_DialogData.Param ();
